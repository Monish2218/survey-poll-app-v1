...
import { PollSchema, surveySchema } from '@/lib/types';
import { z } from 'zod';

function Form({type}:{type:string}) {
    const formSchema = type==="poll" ? PollSchema : surveySchema ;
    type FieldValues = z.infer<typeof formSchema>;
    const { register, control, handleSubmit, formState: { errors }} = useForm<FieldValues>({ resolver: zodResolver(formSchema), defaultValues:{
        description: "",
        title: "",
        options: [{option: ""}]
    }});
  ...
  return (
    <form className='bg-slate-100 w-2/3 pt-4 border-2 border-sky-500 p-8' onSubmit={handleSubmit(onSubmit)}>
        ...
        <input className='w-full mt-2 border-orange-400 border p-2' id="question" placeholder='Type Your Question here' {...register('title')} /><br />
        ...
        {fields.map((field, index)=>{
            return  (
                <>
                <input placeholder='Option' className='w-4/5 mt-1 border-orange-400 border p-2' key={field.id} {...register(`options.${index}.option`)} />
